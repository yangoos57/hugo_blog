...
Surprise는 발생 확률의 역수를 의미한다. 1/8 확률이 예상된다면 해당 현상의 surprise는 8이 된다.  Surprise가 높다는 말은 그만큼 발생 가능성이 낮다는 뜻이므로 통계적으로 많은 정보량을 가지고 있음을 보여준다. Surprise라는 개념을 더 알고싶거나 이를 활용해 정보 엔트로피를 이해하고 싶다면 [StatQuest youtube](https://www.youtube.com/watch?v=YtebGVx-Fxw)를 추천한다.
...
앞선 설명에서 정보 엔트로피는 정보량의 기댓값이라 했다. 그러면 기댓값이 무엇인지, 정보 엔트로피 공식이 어째서 정보량의 기댓값을 구하는 식이 되는지 살펴보겠다.

<br>

주사위 2개를 던져 나온 값의 합계의 이산확률분포를 통해 기댓값을 계산해보자. 아래 차트는 주사위 2개를 던진 값의 합에 따라 나올 수 있는 확률을 보여준다. x축은 주사위의 합을 y축은 합이 나올 확률이 표시되어있다. 차트를 보고 주사위 합이 7이 될 확률은 6/36이 된다는 것을 알 수 있다.
...
기댓값은 확률변수가 취할 수 있는 모든 값들의 평균이다. 공식에서 x는 주사위의 합(2~12)을 의미하고 p(x)는 합이 나올 확률을 의미한다. 기댓값 공식을 활용해 실제로 주사위 2개의 합에 대한 기댓값을 구하자.
...
주사위 2개에 대한 이산확률분포의 기댓값은 7이다. 주사위를 무한번 던지면 나온 값들의 평균이 7에 수렴한다는 의미이다. 기댓값은 우리가 자주사용하는 평균과 같은 의미로 활용된다. 다만 기댓값은 구하고자 하는 대상이 확률 변수이다.
